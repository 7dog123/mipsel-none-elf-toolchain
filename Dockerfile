...
RUN apt-get update && apt-get install -y --no-install-recommends \
    build-essential wget xz-utils file zlib1g-dev tar autoconf automake \
    texinfo libmpc-dev libmpfr-dev libgmp-dev && rm -rf /var/lib/apt/lists/* 
...
RUN mkdir gcc_mipsel && cd gcc_mipsel && \
    ...
    --disable-docs --disable-nls --disable-libada --disable-libssp --disable-libquadmath \
    --disable-libstdc++-v3 --with-float=soft --enable-languages=c,c++ --with-gnu-as\
    --with-gnu-ld && \
    make && make install-strip
